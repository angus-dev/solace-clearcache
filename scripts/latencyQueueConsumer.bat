...
@rem ##########################################################################
@rem
@rem  latencyQueueConsumer startup script for Windows
@rem
@rem ##########################################################################
...
@rem Add default JVM options here. You can also use JAVA_OPTS and LATENCY_QUEUE_CONSUMER_OPTS to pass JVM options to this script.
...
@rem Execute latencyQueueConsumer
"%JAVA_EXE%" %DEFAULT_JVM_OPTS% %JAVA_OPTS% %LATENCY_QUEUE_CONSUMER_OPTS%  -classpath "%CLASSPATH%" com.solace.test.Latency_QueueConsumer %CMD_LINE_ARGS%
...
:fail
rem Set variable LATENCY_QUEUE_CONSUMER_EXIT_CONSOLE if you need the _script_ return code instead of
rem the _cmd.exe /c_ return code!
if  not "" == "%LATENCY_QUEUE_CONSUMER_EXIT_CONSOLE%" exit 1
exit /b 1
